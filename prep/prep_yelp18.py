...
"""
This script builds the Yelp dataset
Alternatively, you may use the DL link to dl the env file
"""
...
def load_reviews(fp):
    with open('./corpus/Yelp18/{}.json'.format(fp),'r') as f:
        data = json.load(f)
    return data

def load_set(fp):
    data= []
    with open('./corpus/Yelp18/{}.txt'.format(fp),'r') as f:
